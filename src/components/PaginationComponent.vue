...
<template>
  <div>
    <p class="py-6 text-base font-normal italic text-themeGray-dark">
      {{ paginationStore.displayedLenth }} de {{ paginationStore.paginationLength }} resultados
    </p>
    ...
    <div class="flex flex-col items-center pb-12 pt-7 space-y-2">
        ...
        <ul class="flex flex-row items-center space-x-3">
          <li v-for="i in paginationStore.splitPages(6)" :key="i">
            <button
              ...
              :class="paginationStore.current_page == i ? 'w-10 bg-themePurple' : 'bg-[#c4c4c4]/75'"
      ...
      <p class="font-normal text-base text-themeGray-dark">página {{ paginationStore.current_page }} de {{ paginationStore.splitPages(6) }}</p>
    </div>
  </div>
</template>
